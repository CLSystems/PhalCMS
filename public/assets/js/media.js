...
UIkit.upload('.js-upload', {
    ...
    // url: 'https://tuinkasmontage.nl/upload/' + subDirs,
    multiple: true,
    mime: 'image/*',
...
$('#toggle-file a').on('click', function (e) {
    e.preventDefault();
    var input = $(this).next();
    ...
    if (input.val().length)
    {
        if (0 < subDirs.length)
        {
            window.location = baseUri + '/media/index/' + subDirs + '?filename=' + encodeURIComponent(input.val());
        }
        else
        {
            window.location = baseUri + '/media/index?filename=' + encodeURIComponent(input.val());
        }
    }
});