...
<form id="admin-list-form" action="{{ uri.getActive() }}" method="post">
    ...
    <table class="uk-table uk-table-small uk-table-hover uk-table-middle uk-table-divider">
        ...
        <tr>
            ...
            <th class="uk-table-expand uk-text-nowrap uk-visible@m">
                {{ partial('Grid/Sort', ['text': _('route'), 'column': 'slug', 'activeOrder' : activeOrder]) }}
            </th>
        ...
        {% for item in paginate.items %}
            <tr>
                ...
                <td>
                    ...
                    {% else %}
                        <a class="uk-link-reset edit uk-icon" uk-icon="icon: pencil" href="{{ uri.routeTo('edit/' ~ item.id) }}">
                            {{ item.title }}
                        </a>
                    {% endif %}
                </td>
