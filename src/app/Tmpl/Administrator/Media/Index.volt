<div id="media-container" data-token="{{ helper('Form::getToken') | escape_attr }}"
    ...
    <div class="actions-container uk-margin">
        <ul class="uk-subnav uk-subnav-divider">
            ...
            <li>
                <a class="uk-text-primary" href="#" uk-toggle="target: #toggle-file">
                    <span uk-icon="plus"></span>
                    {{ _('search-file') }}
                </a>
            </li>
        ...
        <div class="uk-margin" id="toggle-file" hidden>
            <div class="uk-inline">
                <a class="uk-form-icon uk-form-icon-flip search-icon uk-icon" href="#" uk-icon="icon: search"></a>
                <input class="uk-input" type="text" placeholder="{{ _('enter-file-name') | escape_attr }}" id="file-search"
                       onkeyup="this.value = this.value.replace(/[^a-z0-9]/gi, ''); if (event.key === 'Enter' && this.value.length) document.querySelector('#toggle-file a').click();"/>
            </div>
        </div>
