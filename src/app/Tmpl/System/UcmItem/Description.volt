...
{% if ucmItem.prefUrl %}
    {% set EncodedUrl = base64_encode(ucmItem.prefUrl) %}
    <a href="{{ ucmItem.prefUrl }}" target="_blank" rel="noreferrer noopener" class="uk-button uk-button-primary">{{ _('click-here-for-discount') | escape_attr }}</a>
{% endif %}

{% if ucmItem.tags %}
    <div class="uk-margin">
        <div class="widget-title">
            <h3 class="uk-heading-line uk-text-bold"><span>{{ _('post-tags-title') | escape_attr }}</span></h3>
        </div>
        {% for tag in ucmItem.tags %}
            {% set TagObj = tag %}
            <div class="uk-margin-small">
                <a href="/search?tag={{ TagObj.slug }}">{{ TagObj.title }}</a>
            </div>
        {% endfor %}
    </div>
{% endif %}