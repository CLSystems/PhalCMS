...
{% set allowComment = false %}
{% set commentAsGuest = false %}
{% set commentWithEmoji = false %}
{% set canCommentNow = false %}
{% set headerInput = false %}

{#{% if !isEmpty(ucmItem.params) %}#}
...
<div class="comments-container" data-reference-context="{{ commentInstance.referenceContext | escape_attr }}"
    ...
    {% endmacro %}
    {%- macro commentForm (id, type, headerInput, commentWithEmoji) %}
        <div class="uk-background-muted uk-padding-small comment-form-container uk-margin uk-hidden"
            ...
            {% endif %}
            <div class="uk-margin">
                <textarea class="uk-textarea{{ commentWithEmoji ? ' input-emoji' : '' }}" name="userComment" rows="3"
                          cols="15"
