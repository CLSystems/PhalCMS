...
class AdminUcmCommentController extends AdminControllerBase
{
	...
	{
		$this->tag->title(Text::_($this->model->referenceContext . '-comment-admin-index-title'));
	}
	...
	protected function editTitle()
	{
		if ($this->model->id)
		{
			$this->tag->title(Text::_($this->model->referenceContext . '-comment-admin-edit-title', ['userName' => $this->model->userName]));
		}
		else
		{
			$this->tag->title(Text::_($this->model->referenceContext . '-comment-admin-add-title'));
		}
	}
