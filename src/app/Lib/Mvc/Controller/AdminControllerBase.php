...
class AdminControllerBase extends ControllerBase
{
    ...
    {
        $this->tag->title(Text::_(str_replace('_', '-', $this->dispatcher->getControllerName()) . '-index-title'));
    }
    ...
    protected function editTitle()
    {
        if ($this->model->id) {
            ...
            $this->tag->title(Text::_(str_replace('_', '-', $this->dispatcher->getControllerName()) . '-edit-title', $placeholders));
        } else {
            $this->tag->title(Text::_(str_replace('_', '-', $this->dispatcher->getControllerName()) . '-add-title'));
        }
    }
    ...
    public function indexAction()
    {
            ...
            if (empty($filterData['search'])) {
                ...
                if (strpos($sessionFilterData['search'], 'id:') === 0) {
                    ...
                    foreach ($searchFields as $key => $searchField) {
						// Use key to make unique placeholder (It's a PDO thing...)
                        $orWhere[] = 'LOWER(item.' . $searchField . ') LIKE :search' . $key . ':';
						$bindData['search'. $key] = '%' . $sessionFilterData['search'] . '%';
                    }
        ...
        if ($order = $this->request->get('_sort', ['string'], 'id DESC')) {
        ...
        $this->view->setVars(
            [
                ...
                'searchTools'  => $this->view->getPartial(
                    'Grid/SearchTools',
                    [
    ...
    protected function saveTranslations(ModelBase $model, Form $form, array $rawData, $groupField = null)
    {
        ...
        if (!in_array($refTable, $removedTables)) {
            ...
            $db->execute(
                'DELETE FROM ' . $prefixTable . 'translations WHERE translationId LIKE :translationId',
                [
