...
class AdminWidgetController extends AdminControllerBase
{
	...
	protected function saveWidget($widgets)
	{
		...
		if (empty($formData['name'])
			...
			|| empty($widgets['CLSystems\\PhalCMS\\Widget\\' . $formData['name'] . '\\' . $formData['name']])
		)
		{
		...
		else
		{
			...
			$widgetConfig = $widgets['CLSystems\\PhalCMS\\Widget\\' . $formData['name'] . '\\' . $formData['name']];
	...
	{
		$this->tag->title(Text::_('sys-widgets'));
