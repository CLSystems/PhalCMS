...
class SearchController extends ControllerBase
{
    public function resultsAction()
    {
        ...
        {
            $hasQuery   = true;
            $q          = addcslashes(strtolower(str_replace(' ', '%', $q)), '\'');
...
//            ];
        ...
        $queryBuilder->andWhere("item.context IN ('post')")   // {contexts:array} , ['contexts' => $contexts]
            ->orderBy('item.id ASC');
...
//            var_dump($queryBuilder->getPhql()); die;
            $paginator = new Paginator(
                [
                    ...
                    'page'    => $this->request->get('page', ['uint'], 1),
                ]
            );
        ...
        $metadata->metaTitle     = Text::_('search-results-for') . ' ' . $q ?? $tag;
        $metadata->metaDesc      = Text::_('search-results-for') . ' ' . $q ?? $tag;
        $metadata->metaKeys      = Text::_('');
